...
const Review = () => {
    ...
    useEffect(() => {
        fetch("https://raw-relish-server.vercel.app/review")
        .then((res) => res.json())
                ...
                >
                    {reviews.map((review) => (
                        <SwiperSlide key={review._id}>
                            <img
                                style={{ borderRadius: "400px 0px 00px 40px" }}
                    ...
                    {reviews.map((review) => (
                        <SwiperSlide key={review._id}>
                            <div
                                ...
                                className="h-full p-10 bg-transparent border  flex flex-col justify-center items-center  "
                            >
                                <FaQuoteLeft className="h-12 w-16 mb-2 items-center text-yellow-300"></FaQuoteLeft>

                                <h4 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-2 md:mb-4 lg:mb-6  text-white  p-5">
                                    {review.name}{" "}
                                </h4>
